    ...
    <nav className="flex flex-col sm:flex-row items-center justify-between gap-3 px-1 py-4" aria-label="Pagination">
      ...
      <div className="flex items-center gap-1">
        ...
        {pages.map((pageNumber) => (
          <button type="button" key={pageNumber} disabled={loading} onClick={() => onPageChange(pageNumber)} className={`min-w-8 px-2 py-1.5 rounded-lg text-xs font-bold ${pageNumber === page ? "bg-slate-900 text-white" : "border border-slate-200 text-slate-600"}`}>
            {pageNumber}
          </button>
        ))}
